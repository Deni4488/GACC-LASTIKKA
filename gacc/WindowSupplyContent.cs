...
namespace gacc
{
    public partial class WindowSupplyContent : Form
    {
        int supply_id;
        int shop_id;
        // Инициализация формы
        public WindowSupplyContent(int supply_id, int shop_id)
        {
            InitializeComponent();
        ...
        private void Window_Load(object sender, EventArgs e)
        {
            ...
            dgvData.Rows.Clear();
            DBImage.qscldt = DBImage.qsclta.GetDataByByID(supply_id);
            if (DBImage.qscldt.Rows.Count > 0)
            {
                for (int i = 0; i < DBImage.qscldt.Rows.Count; i++)
                {
                    dgvData.Rows.Add();
                    dgvData.Rows[dgvData.RowCount - 1].Cells[0].Value = DBImage.qscldt.Rows[i].ItemArray[1];
                    dgvData.Rows[dgvData.RowCount - 1].Cells[1].Value = DBImage.qscldt.Rows[i].ItemArray[11];
                    dgvData.Rows[dgvData.RowCount - 1].Cells[2].Value = DBImage.qscldt.Rows[i].ItemArray[12];
                    dgvData.Rows[dgvData.RowCount - 1].Cells[3].Value = DBImage.qscldt.Rows[i].ItemArray[6];
                    dgvData.Rows[dgvData.RowCount - 1].Cells[4].Value = DBImage.qscldt.Rows[i].ItemArray[2];
                }
            }
        }
        // -----------------------------------------------------------
        ...
        private void tsbtnSave_Click(object sender, EventArgs e)
        {
            ...
            if (Authorization.Access > 2)
            {
                ...
                if (selected_row >= 0)
                {
                    ...
                    DBImage.supcta.ClearSupply(supply_id);
                    ...
                    if (dgvData.RowCount > 0)
                    {
                        for (int i = 0; i < dgvData.RowCount; i++)
                        {
                            ...
                            DBImage.supcta.InsertQuery(supply_id, good_id, count);
